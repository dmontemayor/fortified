...
contains
  subroutine note(msg)
    ...
    write(*,*)'molreader NOTE: '//msg
  end subroutine note
  !------------------
  subroutine warn(msg,recover)
    character*(*),intent(in)::msg
    character*(*),intent(in),optional::recover

    write(*,*)'molreader WARNING: '//msg
    if(present(recover))then
       write(*,*)'    Program will recover by: '//recover
    else
       stop
    end if
...
!>\details reads the next chunk of the pdb file.
  subroutine refresh_buffer
    integer(long)::irecord,ierr,pdblinecount!,curr_time(8)
